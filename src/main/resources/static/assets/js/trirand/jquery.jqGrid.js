...
//정인선			autowidth: false,
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			ral = cm.align, result="style=\"", clas = cm.classes, nm = cm.name, celp, acp=[], rowspan="";
			...
			result += (clas !== undefined ? (" class=\""+clas+"\"") :"") + ((cm.title && tv) ? (" title=\""+$.jgrid.stripHtml(tv)+"\"") :"") + " kai-data=\""+$.jgrid.stripHtml(tv)+"\"" + " kai-rowspan=\""+rowspan+"\"";
...
//			}	
		...
		//정인선
		if (typeof pin.width === 'string' || pin.width instanceof String)
			grid.hDiv.style.width = pin.width;
...
//					}
		...
		//정인선
		let widthStr = (grid.width - bstw)+"px";
		if (typeof pin.width === 'string' || pin.width instanceof String)
			widthStr = pin.width;
		...
		$(grid.bDiv)
			...
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: widthStr})
		...
		//정인선
		if (typeof pin.width === 'string' || pin.width instanceof String)
			$(grid.bDiv).find("table").first().css({width:pin.width});
...
$.jgrid.extend({
	...
	setSelection : function(selection, onsr, e, isHight) {
		return this.each(function(){
			var $t = this, stat,pt, ner, ia, tpsr, fid, tfid,
...
//				}
	...
	getRowData : function( rowid, usedata, treeindent, visibleTreeNodes ) {
		...
		try{
			...
			res=null, resall=null, getall=null, len=null, j=null;
		}
...
//			afterSaveJqFlag(t, iRow, t[0].p.basedata[iRow-1]);
	...
	addRowData : function(rowid, rdata, pos, src, addclass) {
		...
		var success = false, nm, rnc="", msc="", del="", gi, si, ni,sind, i, v, prp="", aradd, cnm, data, cm, id;
		try{
			// 정인선 신규데이터 상태값 C 추가
...
//					t.updatepager(true,true);
			...
			success = null, nm = null, rnc = null, msc = null, del = null, gi = null, si = null, ni = null,sind = null, i = null, v = null, prp = null, aradd = null, cnm = null, data = null, cm = null, id = null;
		}
	...
	showHideCol : function(colname,show) {
		return this.each(function() {
			var $t = this, fndh=false, brd=$.jgrid.cell_width ? 0: $t.p.cellLayout, cw, frozen = false, sw, gHead, gh;
			try{
				if (!$t.grid ) {return;}
				...
				$(this.p.colModel).each(function(i) {
					if ($.inArray(this.name,colname) !== -1 && this.hidden === sw) {
						...
						$("#"+$.jgrid.jqID($t.p.id)+" tbody > .jqgfirstrow > td ").eq( i ).css("display", show);
						$($t.rows).each(function(){
							if (!$(this).hasClass("jqgroup")) {
								if(sw){
									if( $(this.cells[i]).attr('kai-rowspan') === ''){
										$(this.cells[i]).css("display", show);
									}
								}
								else
				...
				*/
				...
				$t = null, fndh = null, brd = null, cw = null, frozen = null,sw = null,gHead = null, gh = null;
			}
		});
	},
				...
				if (ln && ln>0){
					...
					for(; i < ln; i++){
						if($($t.rows[i]).hasClass('jqgrow') && $t.rows[i].id !== "norecs") {
							...
							if($t.p.frozenColumns && pos <= $t.p.frozenColCount) { // frozen columns
								$("#"+$t.rows[i].id+" td[role='gridcell']", "#" + $.jgrid.jqID($t.p.id + "_frozen")).eq(pos).html(vl).attr(title);
							}
						}
					}
				}
...
//			$t.updatepager(true,false);
	...
	customFrozenColumns: function () {
		return this.each(function() {
			if ( !this.grid ) {return;}
			var ts = this;
			var tbody = $(ts).find("tbody").first().get(0);
...
//					$($t).triggerHandler("onFrozenColumnsCall");
				...
				cm = null,i=null, len = null, hd= null, hover = null, borderbox = null, pixelfix = null;
			}
